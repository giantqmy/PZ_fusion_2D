...
class WTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        # 确保在加载预训练权重后，requires_grad 不会被修改
        self.wt_filter.requires_grad_(False)
        self.iwt_filter.requires_grad_(False)

        self.wt_function = partial(wavelet_transform, filters=self.wt_filter)
        self.iwt_function = partial(inverse_wavelet_transform, filters=self.iwt_filter)

        # 计算 padding 值，确保输出尺寸与输入相同
        ...
        self.wavelet_convs = nn.ModuleList(
            [nn.Conv2d(in_channels * 4, in_channels * 4, kernel_size, padding=padding, stride=1, dilation=1,
