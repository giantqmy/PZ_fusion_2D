...
from ultralytics.data.utils import normalize_polarization_batch
...
class BasePredictor:
    ...
    def preprocess(self, im):
        ...
        if not_tensor:
            im = normalize_polarization_batch(im)
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        with self._lock:  # for thread-safe inference
            ...
            # Warmup model
            if not self.done_warmup:#qmy
                model_yaml = getattr(getattr(self.model, "model", None), "yaml", {})
                warmup_channels = int(model_yaml.get("ch", self.args.ch))
                self.model.warmup(
                    imgsz=(
