...
# Depth PNGs store metric depth as uint16(depth_in_meters * 256). Statistics
# are computed from nonzero training pixels only; zero denotes missing LiDAR depth.
DEPTH_STORAGE_SCALE = 256.0
DEPTH_TRAIN_MEAN_M = 29.652361724372745
DEPTH_TRAIN_STD_M = 23.830449242515485


def normalize_polarization_batch(images):
    """Normalize a formatted 9-channel polarization/depth batch for the detector."""
    images = images.float()
    if images.ndim != 4 or images.shape[1] != 9:
        return images / 255.0

    normalized = images / 255.0  # channels 1..8 are uint8 visual modalities
    raw_depth = images[:, 0:1]
    valid_depth = raw_depth > 0
    depth_m = raw_depth / DEPTH_STORAGE_SCALE
    depth_zscore = (depth_m - DEPTH_TRAIN_MEAN_M) / DEPTH_TRAIN_STD_M
    normalized[:, 0:1] = torch.where(valid_depth, depth_zscore, torch.zeros_like(depth_zscore))
    return normalized
...
#         # 如果是灰度图，ndim=2，需要扩展到 (H,W,1)
#         if img.ndim == 2:
#             img = np.expand_dims(img, axis=2)

#         # 如果是 (H,W,3)，就保持原样
#         # 如果是 (H,W,4)，你要不要丢掉 alpha 通道？（可以 img = img[:,:,:3]）

#         # print(f"读取 {suffix}: shape={img.shape}, ndim={img.ndim}")

#         if ret_list is None:
#             ret_list = img
#         else:
#             ret_list = np.concatenate((ret_list, img), axis=2)  # 在通道维度拼接
    
#     # 最终检查
#     if ret_list is None or ret_list.shape[2] != 8:
#         raise ValueError(f"Expected 8 channels, got {None if ret_list is None else ret_list.shape[2]}")
...
def getPolarizationImages(base_path, image_mode=None):
    ...
    # Visual modality files live beside the index image. The uint16 depth map
    # lives in the sibling "depth" directory.
    channel_files = [
        # ("_aolp_rgb.png", True),   # 灰度 0 
        # ("_aolp_nir.png", True) ,  # 灰度 1
        # ("_depth.png", True) , # 灰度 2
      
        # ("_NIR_RGB.png", False),  #3 4 5
        # ("_RGB_RGB.png", False)  #678
        ("_S0_rgb.png", False),    # RGB 0 1 2  8 7 6
        ("_S0_nir.png", True),     # 灰度 3       5
        ("_dolp_rgb.png", True),   # 灰度 4       4
        ("_dolp_nir.png", True),   # 灰度 5       3 
        ("_aolp_rgb.png", True),   # 灰度 6       2
        ("_aolp_nir.png", True),  # 灰度 7       1
    ]
    ...
    if not os.path.exists(depth_path):
        raise FileNotFoundError(f"Missing required uint16 depth file: {depth_path}")
    depth = cv2.imread(depth_path, cv2.IMREAD_UNCHANGED)
    if depth is None:
        raise ValueError(f"Failed to read depth image: {depth_path}")
    if depth.ndim != 2 or depth.dtype != np.uint16:
        raise ValueError(f"Expected uint16 single-channel depth image at {depth_path}, got {depth.shape}, {depth.dtype}")
    ret_list = np.concatenate((ret_list, depth.astype(np.float32)[..., None]), axis=2)
    # print("aaaaa")
...
#     # 文件后缀 + 是否灰度
#     channel_files = [
#         # ("_S0_rgb.png", False),    # RGB
#         ("_S0_nir.png", True)    # 灰度
#         # ("_dolp_rgb.png", True),   # 灰度
#         # ("_dolp_nir.png", True),   # 灰度
#         # ("_aolp_rgb.png", True),   # 灰度
#         # ("_aolp_nir.png", True)    # 灰度
#     ]
...
#     # print(ret_list.shape)
#     # 最终检查
#     if ret_list.shape[2] != 1:
#         raise ValueError(f"Expected 4 channels, got {ret_list.shape[2]}")
