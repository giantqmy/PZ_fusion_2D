...
def _make_polarization_canvas(shape, dtype):
    """Create a padding canvas while preserving the custom depth channel."""
    canvas = np.full(shape, 114, dtype=dtype)
    if len(shape) == 3 and shape[2] == 9:
        canvas[..., -1] = 0
    return canvas


def _warp_polarization_channels(img, matrix, size, perspective):
    """Warp 9-channel polarization data with zero padding for depth."""
    warped = []
    for channel_index in range(img.shape[2]):
        border_value = 0 if channel_index == img.shape[2] - 1 else 114
        channel = (
            cv2.warpPerspective(img[:, :, channel_index], matrix, dsize=size, borderValue=border_value)
            if perspective
            else cv2.warpAffine(img[:, :, channel_index], matrix[:2], dsize=size, borderValue=border_value)
        )
        warped.append(channel[..., None])
    return np.concatenate(warped, axis=2)
...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic3(self, labels):
        ...
        for i in range(3):
            ...
            # Place img in img3
            if i == 0:  # center
                img3 = _make_polarization_canvas((s * 3, s * 3, img.shape[2]), img.dtype)
                h0, w0 = h, w
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            # Place img in img4
            if i == 0:  # top left
                img4 = _make_polarization_canvas((s * 2, s * 2, img.shape[2]), img.dtype)
    ...
    def _mosaic9(self, labels):
        ...
        for i in range(9):
            ...
            # Place img in img9
            if i == 0:  # center
                img9 = _make_polarization_canvas((s * 3, s * 3, img.shape[2]), img.dtype)
                h0, w0 = h, w
                c = s, s, s + w, s + h  # xmin, ymin, xmax, ymax (base) coordinates
            elif i == 1:  # top
                c = s, s - h, s + w, s
...
class RandomPerspective:
    ...
    def affine_transform(self, img, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if img.ndim == 3 and img.shape[2] == 9:
                img = _warp_polarization_channels(img, M, self.size, bool(self.perspective))
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        for i in range(img.shape[2]):
            border_value = 0 if img.shape[2] == 9 and i == img.shape[2] - 1 else 114
            tmp = np.expand_dims(cv2.copyMakeBorder(img[:, :, i], top, bottom, left, right, cv2.BORDER_CONSTANT,
                                                    value=border_value), 2)  # add border
            if i == 0:
                tmp_img = tmp
            else:
