...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        # im = im[:, :, :3].copy()
        # im = im[:, :, [0, 2]].copy()
        # # 添加一个全 0 的第三通道，构造 3 通道图像
        # h, w = im.shape[:2]
        # zero_channel = np.zeros((h, w), dtype=im.dtype)

        # 拼接成为伪3通道图
        # im = np.dstack((im, zero_channel))

        # s0 = (im[:, :, 0].astype(np.float32) + im[:, :, 2].astype(np.float32)) / 2.0
        # # 转换为三通道灰度图像
        # im = np.stack([s0, s0, s0], axis=-1)  # 创建三个相同的通道
