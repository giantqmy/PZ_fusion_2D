...
GT_PATH = os.path.join(os.getcwd(), '/media/ddc/新加卷/hys/qmy/PZ_8/AP/valgt')
DR_PATH = os.path.join(os.getcwd(), '/media/ddc/新加卷/hys/qmy/PZ_8/AP/labels+rgb+aolp+dolp+depth')
# if there are no images then no animation can be shown
# IMG_PATH = os.path.join(os.getcwd(), 'input', 'images-optional')

IMG_PATH = r"J:\YZ-SOD-3-13K-220428-data\data\valimageall"#好像我这个图片不闪的话就快很多
...
def voc_ap(rec, prec):
    ...
    """
     The Average Precision (AP) is the area under the curveap += ((mrec[i]-mrec[i-1])*mpre[i])
        (numerical integration)
        matlab: ap=sum((mrec(i)-mrec(i-1)).*mpre(i));
    """
    ap = 0.0
    for i in i_list:
    #   if i in i_list[:len(i_list)-1]:
    #     ap += ((mrec[i]-mrec[i-1])*mpre[i])
    #   else:
    #     ap += ((mrec[i]-mrec[i-1])*mpre[i-1])/2
    ...
    if true_p_bar != "":
        ...
        for i, val in enumerate(sorted_values):
            ...
            if i == (len(sorted_values)-1): # largest bar
                adjust_axes(r, t, fig, axes)
    # set window title
    # fig.canvas.set_window_title(window_title)#注释
    ...
    for txt_file in dr_files_list:
        ...
        for line in lines:
            ...
            try:
                ...
                tmp_class_name, left, top, right, bottom, confidence = parts[:6]
            except ValueError:
                error_msg = "Error: File " + txt_file + " in the wrong format.\n"
...
with open(output_files_path + "/output.txt", 'w') as output_file:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        if draw_plot:
            ...
            # set window title
            fig = plt.gcf() # gcf - get current figure
            # fig.canvas.set_window_title('AP ' + class_name)#注释
            # set plot title
